...
// Normalize URL to hostname, consistent with content.js
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.action === 'sendUrl') {
    ...
    const hostname = normalizeUrl(url); // Normalize here to match content.js
    ...
    // Forward selected account data to content script
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      if (tabs[0]?.id) {
